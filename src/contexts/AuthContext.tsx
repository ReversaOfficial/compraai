...
export type UserProfile = CustomerProfile | SellerProfile;
...
const updateStoredUser = (id: string, data: Partial<UserProfile>) => {
  const users = getAllUsers();
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signInSeller = async (email: string, password: string): Promise<{ error: string | null }> => {
    const users = getAllUsers();
    const found = users.find(u => u.email === email && u.role === 'seller');
